...
<script language='javascript'>
	function menu_select() {
		if (product_list.menu.value == 'DC01') { location.replace("Detergent_C.php?code=lavadura"); }
		if (product_list.menu.value == 'DC02') { location.replace("Detergent_C.php?code=assistance"); }
		if (product_list.menu.value == 'DC03') { location.replace("Detergent_C.php?code=fiber"); }
	}
</script>
<script>
...
<?		
	if(!strcmp($code, 'lavadura')) {
		$sql = "select * from product_list where small_class LIKE '본세탁'";
	}else if(!strcmp($code, 'assistance')) {
		$sql = "select * from product_list where small_class LIKE '세탁보조'";
	}else{
		$sql = "select * from product_list where small_class LIKE '섬유유연제'";
	} 
    ...
    <div data-role="page" id="background">
        <div data-role="header" data-position="fixed" style="background-color: #7ccbc8; color: white;">
            ...
            <div data-role="navbar">
                <ul class="header_ul">
                    <li><a href="Detergent_A.php">주택용</a></li>
                    <li><a href="Detergent_B.php">주방용</a></li>
                    <li><a href="Detergent_C.php?code=lavadura" class="ui-btn-active">세탁용</a></li>
                </ul>
            </div>
        </div>
                ...
                <div class="list_wrap">
					<div class='sel'>
						<select name = "menu" onChange="javascript:menu_select()">
							<option value='DC01' <? if(!strcmp($code, 'lavadura')) { echo "selected='selected'"; } ?>>본세탁 (<? echo "$count[0]"?>)</option>
							<option value='DC02' <? if(!strcmp($code, 'assistance')) { echo "selected='selected'"; } ?>>세탁보조 (<? echo "$count[1]"?>)</option>
							<option value='DC03' <? if(!strcmp($code, 'fiber')) { echo "selected='selected'"; } ?>>섬유유연제 (<? echo "$count[2]"?>)</option>
						</select>
					</div>
                    <?
